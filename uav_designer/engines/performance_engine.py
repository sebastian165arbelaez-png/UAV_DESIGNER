...
Internally consistent formulas built on shared assumptions.
...
def estimate_CD0(configuration: str = "Conventional Tractor") -> float:
    """
    Wetted-area method CD0 estimates by configuration.
    These are intentionally labelled as approximations.
    """
    base = {
...
def compute_performance(total_mass_kg: float,
    ...
    """
    Master performance computation.
    Returns a rich dict of all performance metrics and sweep arrays.
    """
    ...
    CL_max  = af["CL_max"] * 0.90    # finite wing reduction

    W = total_mass_kg * G
    S = gd.S

    # ── Key CL values ──────────────────────────────────────────────────────
    CL_md   = math.sqrt(math.pi * e * gd.AR * CD0)          # min drag
    CL_be   = math.sqrt(3 * math.pi * e * gd.AR * CD0)      # best endurance (electric)
    ...
    V_br    = V_md                    # best range speed ≈ min drag speed for electric

    # ── Thrust required sweep ──────────────────────────────────────────────
    ...
    # Thrust available: simplified decay model
    ...
    # Max speed (T_avail = T_req intersection)
    diff    = T_avail - D_arr
    ...
    eta_total  = pd.eta_prop * pd.eta_motor * pd.eta_esc if hasattr(pd, 'eta_motor') else 0.65
    eta_total  = max(eta_total, 0.40)
    P_elec_cr  = P_shaft_cr / eta_total + extra_load_w

    # ── Endurance ──────────────────────────────────────────────────────────
    # Also compute endurance sweep
    ...
    T_end  = (pd.energy_wh / P_end) * 60   # minutes

    end_max    = float(np.max(T_end))
    V_end_max  = float(V_end[np.argmax(T_end)])
    end_cruise = (pd.energy_wh / P_elec_cr) * 60 if P_elec_cr > 0 else 0

    # ── Drag polar arrays for plot ─────────────────────────────────────────
